### Hi, I'm Shahar 👋

CS student who likes building things that actually get used: AI agents, automations, and apps that solve real problems.

**What I've built:**

🐾 **[PuppyCare](https://github.com/ShaharKoza/PuppyCare)** is a smart kennel system I built for monitoring dogs. Raspberry Pi sensors stream live data to Firebase, and an iOS app I wrote in SwiftUI shows temperature, motion and camera snapshots, with push alerts when something is wrong. The Pi code lives in [smart-kennel](https://github.com/ShaharKoza/smart-kennel).

**Client work** (private repos):

- Audit and automation backend for a networking company
- Hebrew appointment booking site for a barbershop, with SMS login (Next.js, TypeScript, Supabase)
- Business website for a construction and project management company

**Tools I work with:** Python, TypeScript, Swift, Next.js, Firebase, Supabase, Raspberry Pi

📫 [LinkedIn](https://www.linkedin.com/in/shahar-kozniak) | kshahar1315@gmail.com
